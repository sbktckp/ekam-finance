...
type Account  = { id: string; name: string; color: string; type: string; currency: string }
...
export function AddTransactionModal({ open, onClose, accounts, categories }: Props) {
  ...
  return (
    <>
          ...
          <form action={handleSubmit} className="px-6 pb-6 pt-4 space-y-4 overflow-y-auto max-h-[75vh] sm:max-h-none">
            ...
            {accounts.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-gray-200 p-5 text-center">
                ...
                <button
                  type="button" onClick={() => setAddAccOpen(true)}
              ...
              <>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    isExpense
                      ? 'bg-red-500 text-white hover:shadow-red-500/25'
                      : 'text-black hover:shadow-emerald-500/25'
                  )}
      ...
      {/* Nested AddAccount modal for when no accounts exist */}
      <AddAccountModal
        open={addAccOpen}
